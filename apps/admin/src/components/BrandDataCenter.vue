...
<template>
  <section class="brand-data-page" v-loading="loading">
    ...
    <template v-else-if="activeTab === 'assets'">
      <div class="workspace-heading"><div><h3>素材库</h3><p>按型号、用途、功能、场景、动作和景别建立AI剪辑索引。</p></div><div><el-button @click="rebuildAssetIndex">重建AI索引</el-button><el-button :icon="Refresh" @click="syncAssets">扫描同步</el-button><el-button type="primary" :icon="Plus" @click="openBatchUpload">上传素材</el-button></div></div>
      ...
      <template v-else-if="assetView === 'list' || assetView === 'review'">
        ...
        <div class="bulk-toolbar"><span>已选 {{ selectedAssets.length }} 条</span><el-button size="small" type="primary" :disabled="!selectedAssets.length" @click="openAssetBulk">批量修改</el-button><el-button size="small" type="success" :disabled="!selectedAssets.length" @click="bulkAssets('APPROVE')">批量通过</el-button><el-button size="small" :disabled="!selectedAssets.length" @click="bulkAssets('REANALYZE')">批量重分析</el-button><el-button size="small" type="danger" plain :disabled="!selectedAssets.length" @click="bulkAssets('ARCHIVE')">批量删除</el-button></div>
