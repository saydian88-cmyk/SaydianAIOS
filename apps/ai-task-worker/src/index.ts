      ...
      return {
        ...
        properties: {
          ...
          ...(batchCodexDirectFullVideo ? {
            batchResults: {
              type: "array",
              minItems: Math.max(1, batchExpectedResultCount),
              items: {
                type: "object",
                additionalProperties: false,
                properties: {
...
function prompt(taskPackage: JsonRecord, detectedSkill: DetectedSkill) {
  ...
  if (isReferenceDirectFullVideo) {
    ...
    return [
      ...
      "REFERENCE_MEDIA_PRECHECK: Before any script, material selection, or render work, read and execute G:\\codex\\xcodeplace\\CodexHome\\skills\\reference-video-audio-extraction\\SKILL.md. Write its real reference-video.mp4, reference-original.wav and extraction-evidence.json under this task workspace's reference-media directory, then hand them to the full editing Skill. This is required even when the final spoken content is revoiced, because usable reference BGM, ambience, effects, structure and beat map must be analysed from the real source.",
      "REFERENCE_BROWSER_RECOVERY: A direct downloader 'Fresh cookies' response is not a terminal result. The user is logged into Douyin in the Codex in-app browser, so use browser:control-in-app-browser only: open and play the supplied reference page there, obtain the already-played media stream without reading, exporting, displaying or persisting browser cookies, then run extract_reference_media.py --browser-stream-stdin to save the local reference media. Google Chrome is forbidden unless the user explicitly requests it. Do not stop after an empty initial network observation; complete the documented playback and extraction sequence, record every attempt in extraction-evidence.json, and continue whenever a real media file is recovered.",
