...
function prompt(taskPackage: JsonRecord, detectedSkill: DetectedSkill) {
  ...
  if (isReferenceDirectFullVideo) {
    ...
    return [
      ...
      "REFERENCE_BROWSER_RECOVERY: A direct downloader 'Fresh cookies' response is not a terminal result. Follow the extraction Skill's logged-browser recovery in the current Chrome session: open and play the supplied reference page, obtain the already-played media stream without reading, exporting, displaying or persisting browser cookies, then run extract_reference_media.py --browser-stream-stdin to save the local reference media. Do not stop after an empty initial network observation; complete the documented playback and extraction sequence, record every attempt in extraction-evidence.json, and continue whenever a real media file is recovered.",
