...
function compileBatchImagePostPrompt(project: Record<string, any>, brief: Record<string, unknown>) {
  ...
  const additionalPrompt = String(batch.additionalPrompt || "").trim();
  return [
    `批量图文执行清单（每项为 1 组，必须按此执行，不可调换产品或类型）：\n${groupLines}`,
    "交付：每组同步完成至少 5 张图文页（1 张封面和至少 4 张内容页）、标题、标签和发布文案；每页必须对应不同的真实图片文件，少于 5 张或复用图片均不能算完成。",
    "发布文案：按小红书笔记风格写 120-260 个中文字符，分成数个自然短段，使用 2-5 个贴合语境的常用符号或表情；要有场景钩子、实用价值和自然互动收尾。换行必须使用真实换行符，禁止输出 \\n、\\r、\\t 等代码形式的转义文字。",
    "素材限制：仅使用对应产品的真实素材，不得混用其他产品素材。",
    "对外内容限制：最终图片、标题、标签和发布文案不得出现图文类型、页数、组号、任务编号或其他内部制作标识。",
    "差异要求：每组的钩子、文案、页面逻辑和主图片源必须不同；不能只更换文案、颜色或裁切。",
    "截帧要求：所有视频截帧必须先调色提亮；暗、灰、模糊或对比不足的帧直接淘汰。",
    "实拍类要求：有可用素材时，每组至少包含一张多功能实拍拼图，由 4–8 张不同真实照片或调色后截帧组成，不能全部是单图页面。",
    ...(additionalPrompt ? [`补充要求：${additionalPrompt}`] : []),
  ].join("\n");
}
