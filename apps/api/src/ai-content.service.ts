...
import {
  ...
  buildBailianEditingVideoContext,
...
本次输入中的assets已经由系统预过滤，只包含可作为剪辑主镜头的VIDEO素材索引。图片、音频、文档和包装资源均未提供，不得在返回中引用或猜测这些资源。
assetIds必须等于所有shotRequirements.matchedVideoAssetIds去重后的并集；两者只能从availableVideoAssetIds选择。每条auxiliaryImageAssetIds必须固定返回空数组。同一VIDEO素材默认只能绑定一条口播/字幕行；当前返回格式不支持可核验的不同起止片段，因此不得用同一个assetId覆盖多句，没有不同的直接视频证据时必须标记NEED_SHOOT。
没有合适视频素材时，matchedVideoAssetIds返回空数组，assetStatus返回NEED_SHOOT，并在materialGaps写清补拍需求；禁止虚构素材ID、对象路径或文件名。
输入：${JSON.stringify(bailianContext)}`,
    );
