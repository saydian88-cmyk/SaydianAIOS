...
export class WorkbenchService {
  ...
  async videoLibrary(query: Record<string, string | undefined>) {
    ...
    const [items, total, categoryRows, productModelRows] = await Promise.all([
      ...
      where,
      include: {
        outputAsset: { select: { id: true, displayName: true, fileName: true, durationSeconds: true, width: true, height: true, objectKey: true, sourceSnapshot: true, reviewStatus: true, availabilityStatus: true, deletedAt: true, aiTaskOutputs: { select: { kind: true, metadata: true, aiTask: { select: { instructions: true } } } } } },
        contentPlan: { select: {
