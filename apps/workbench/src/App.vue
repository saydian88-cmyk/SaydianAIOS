...
<template>
  ...
  <div v-else class="workbench">
    <header class="topbar">
      <div>
        <p class="eyebrow">SAYDIAN WORKBENCH</p>
        <h1>{{ active === "home" ? "今日工作" : active === "tasks" ? "任务中心" : active === "live" ? "直播学习与复盘" : active === "messages" ? "消息" : "工作台" }}</h1>
      </div>
        ...
        <section class="split-grid">
          <article class="section-card">
            ...
            <div class="action-grid">
              ...
              <button @click="uploadVisible = true"><el-icon><UploadFilled /></el-icon><span>上传素材</span></button>
              <button @click="knowledgeVisible = true"><el-icon><Collection /></el-icon><span>补充知识</span></button>
    ...
    <nav class="bottom-nav">
      ...
      <button @click="openMall"><el-icon><Shop /></el-icon><span>商城</span></button>
    </nav>
  </div>
