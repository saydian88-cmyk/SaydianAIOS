...
const props = withDefaults(defineProps<{ modelValue?: object | null; placeholder?: string }>(), {
  modelValue: null,
  placeholder: "填写任务内容",
});
...
const editor = useEditor({
  ...
  editorProps: {
    attributes: { class: "task-editor-content", "data-placeholder": props.placeholder },
  },
...
watch(
  ...
  (value) => {
    ...
    if (JSON.stringify(editor.value.getJSON()) !== JSON.stringify(value)) editor.value.commands.setContent(value);
  },
);
...
function setLink() {
  ...
  const href = window.prompt("输入链接（仅支持 http、https 或 mailto）", current);
  if (href === null) return;
...
<style>
.task-editor { border: 1px solid #dcdfe6; border-radius: 8px; overflow: hidden; background: #fff; }
...
.task-editor-content p.is-editor-empty:first-child::before { content: attr(data-placeholder); color: #a8abb2; float: left; height: 0; pointer-events: none; }
</style>
